...
import { Save, Eye, ArrowLeft, Star, Bold, Italic, UnderlineIcon,
         Heading2, Heading3, List, ListOrdered, Quote, Undo, Redo, Image as ImageIcon } from 'lucide-react';
...
export default function EditArticlePage() {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: { class: 'prose prose-sm max-w-none min-h-[360px] px-6 py-5 focus:outline-none text-navy leading-relaxed' },
    },
  });
    ...
    fetch(`${API}/news/${id}`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(article => {
        setForm({
          ...
          coverImage: article.coverImage || '', status: article.status || 'DRAFT',
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-navy">Edit Article</h1>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="card p-5">
            ...
            <textarea value={form.excerpt} onChange={e => update('excerpt', e.target.value)}
              placeholder="Brief excerpt or summary…" rows={2}
              ...
              {editor && (
                <div className="flex items-center gap-0.5 px-3 py-2 border-b border-gray-100 bg-gray-50 flex-wrap">
                  ...
                  <button onMouseDown={e => { e.preventDefault(); editor.chain().focus().undo().run(); }}
                    className="p-1.5 rounded text-gray-500 hover:bg-gray-200"><Undo className="w-3.5 h-3.5" /></button>
                  <button onMouseDown={e => { e.preventDefault(); editor.chain().focus().redo().run(); }}
                    className="p-1.5 rounded text-gray-500 hover:bg-gray-200"><Redo className="w-3.5 h-3.5" /></button>
                </div>
              )}
            ...
            <div className="card p-5 space-y-4">
              ...
              <div>
                ...
                <input value={form.seoTitle} onChange={e => update('seoTitle', e.target.value)}
                  placeholder="Overrides article title in search results" className="input" maxLength={60} />
              ...
              <div>
                ...
                <textarea value={form.seoDescription} onChange={e => update('seoDescription', e.target.value)}
                  placeholder="Brief description shown in search results…" rows={3}
              ...
              <div className="rounded-lg border border-gray-200 p-4 bg-gray-50">
                ...
                <p className="text-gray-500 text-sm mt-0.5">{form.seoDescription || form.excerpt || 'Article description…'}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="card p-4 space-y-3">
            ...
            <input ref={coverRef} type="file" accept="image/*" className="hidden"
              onChange={e => { const file = e.target.files?.[0]; if (file) update('coverImage', URL.createObjectURL(file)); }} />
